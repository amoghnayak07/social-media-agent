...
- **Voice refresh** ("Refresh voice", with an N selector) is an async, chunked, resumable, idempotent background job that learns the creator's voice from their **last N posts** (N is user-defined with a sensible default of ~20 and a capped maximum): it pulls the comments + the creator's own replies from those posts, classifies and pairs them, embeds them, and writes to the voice store. Last-N-posts is used instead of a time window because it's bounded and predictable (a "past year" window could be 40 posts or 4,000 — unsafe to size/chunk on the free tier), it still captures current voice (the most recent posts), and it's a control the creator can tune. The job processes post-by-post (the natural chunk boundary); incremental refresh pulls only posts newer than the most-recently-ingested one. N caps the post count only — a single viral post can have tens of thousands of comments, so per-post pagination is still required. On Render's free tier it runs as a FastAPI background task (no always-on worker); a real task queue is future work.
